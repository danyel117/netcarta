...
export function ArticleScreen({ slug }: { slug: string }) {
  ...
  const actions = useMemo(
    () => (
      <div className="flex flex-wrap items-center gap-3 text-sm">
        ...
        <div className="bevel-inset bg-white px-4 py-2">Current article: {slug.replaceAll("_", " ")}</div>
      </div>
    ),
    [slug],
  );
  ...
  return (
    <EncartaShell title={cached?.title ?? slug.replaceAll("_", " ")} subtitle="Wikipedia content, routed through Convex caching to keep the illusion intact." actions={actions}>
      {loading ? (
        <div className="flex flex-1 items-center justify-center p-10">
          <div className="bevel-inset bg-white px-6 py-5 text-lg">Loading article from Convex cache...</div>
        </div>
      ) : null}
        ...
        <div className="grid flex-1 gap-6 p-6 lg:grid-cols-[1.3fr,0.95fr] lg:p-8">
          <section className="space-y-6">
            <div className="border-2 border-black bg-[rgba(255,255,255,0.82)] p-6">
              <div className="flex flex-col gap-6 md:flex-row">
                <div className="min-w-0 flex-1">
                  <h2 className="font-body text-5xl text-[#111]">{cached.title}</h2>
            ...
            <div className="border-2 border-black bg-[rgba(255,248,220,0.85)] p-5">
              <div className="bg-[#b0b0b0] px-2 py-1 text-2xl font-bold">Did You Know?</div>
              <p className="article-body mt-4 text-[18px] leading-8 text-[#1f2937]">
                Every linked article on this screen can become part of a race path. The fastest route is often not the most obvious one.
              </p>
            </div>
          </section>
          ...
          <section className="space-y-6">
            <div className="border-2 border-black bg-[rgba(0,0,0,0.86)] p-5 text-white">
              <div className="text-xs uppercase tracking-[0.45em] text-[#87dcdc]">See Also</div>
              <div className="link-list mt-4 grid gap-3 text-lg">
                {cached.seeAlso.map((item) => (
                  <Link
                    key={item.slug}
                ...
                {cached.seeAlso.length === 0 ? <div className="text-sm text-[#d1d5db]">No related links surfaced for this article.</div> : null}
              </div>
            </div>

            <div className="border-2 border-black bg-[rgba(255,255,255,0.75)] p-5">
              <div className="bg-[#b0b0b0] px-2 py-1 text-2xl font-bold">Race Tip</div>
              <p className="mt-4 text-[15px] leading-7 text-[#1f2937]">
                In race mode, navigation is locked to this related-link rail. That keeps the challenge fair and turns Wikipedia into a realtime graph puzzle.
              </p>
              <Link href="/race" className="bevel mt-5 inline-flex bg-panel px-4 py-2 font-bold">
                Start A Room
              </Link>
            </div>
          </section>
