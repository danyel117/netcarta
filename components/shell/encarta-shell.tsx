        ...
        <div className="border-b-2 border-black bg-[#d7d7d7] px-3 py-2 text-[12px]">
          ...
          <div className="encarta-toolbar flex flex-wrap gap-2">
            {toolbarItems.map((item) => {
              if (item.disabled) {
                return (
                  <button key={item.label} className="bevel bg-panel px-4 py-2 text-sm" disabled>
                    {item.label}
                  </button>
                );
              }
        ...
        <div className="grid min-h-[calc(100vh-170px)] grid-cols-1 bg-[linear-gradient(135deg,#eff7ff_0%,#f5eedf_38%,#d9e7f6_100%)] lg:grid-cols-[260px,1fr]">
          <aside className="border-r-2 border-black bg-[#efefef] p-4">
            <div className="bevel-inset mb-4 bg-black px-3 py-4 text-white">
              <div className="text-xs uppercase tracking-[0.3em] text-[#8dd3d3]">Encyclopedia</div>
              <h1 className="mt-2 text-3xl font-semibold">{title}</h1>
              {subtitle ? <p className="mt-3 text-sm leading-5 text-[#d4d4d4]">{subtitle}</p> : null}
            </div>

            <div className="space-y-4 text-sm">
              <section>
                <div className="mb-2 bg-[#b0b0b0] px-2 py-1 text-xl font-bold">Explore Netcarta</div>
                <div className="space-y-1 px-2 text-[17px]">
                  <Link href="/articles/Internet" className="block py-1 hover:underline">
                    Featured article
                  </Link>
              ...
              <section>
                <div className="mb-2 bg-[#b0b0b0] px-2 py-1 text-xl font-bold">Session Tools</div>
                <div className="space-y-2 px-2 text-[15px] text-[#202020]">
                  <p>Realtime rooms, cached articles, and a judge-friendly leaderboard live in Convex.</p>
                  <p>Use the top toolbar to jump between the home hub, article chrome, and race flow.</p>
                </div>
              </section>
            </div>
          </aside>

          <main className={clsx("relative flex min-h-full flex-col bg-[radial-gradient(circle_at_top_right,rgba(116,172,222,0.28),transparent_32%),linear-gradient(180deg,rgba(255,255,255,0.55),rgba(255,255,255,0.15))]", actions && "pb-24")}>
            {actions ? <div className="border-b-2 border-black bg-[#d8d8d8] p-3">{actions}</div> : null}
            {children}
          </main>
        </div>
